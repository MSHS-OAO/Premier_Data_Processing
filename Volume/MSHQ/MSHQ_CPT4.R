...
rev_map <- read_excel(paste0("J:\\deans\\Presidents\\SixSigma\\",
                             "MSHS Productivity\\Productivity\\",
                             "Volume - Data\\MSH Data\\Charges\\",
                             "MSHQ REV CROSSWALK.xlsx")) %>%
  select(c(1,5:8)) %>%
  distinct()
# Bring in CPT reference table
# For upload months: January,April,July,October there is a new cpt_ref to download
cpt_ref <- read_excel(paste0("J:\\deans\\Presidents\\SixSigma\\",
                             "MSHS Productivity\\Productivity\\",
                             "Volume - Data\\MSH Data\\RIS\\Mapping\\",
                             "CPT_Ref.xlsx")) %>%
...
charges <- function(MSH, MSQ){
  ...
  MSHQ <- MSHQ %>%
    mutate(END = paste0(substr(START, 6, 7), "/",
                        days_in_month(START), "/", Year),
           START = paste0(substr(START, 6, 7), "/",
                          substr(START, 9, 10), "/", Year),
...
master <- function(){
  ...
  master <- readRDS(paste0("J:\\deans\\Presidents\\SixSigma\\",
                           "MSHS Productivity\\Productivity\\Volume - Data\\",
                           "MSH Data\\Charges\\Master\\master.RDS"))
  # Check that new charge detail does not overlap with master
  if(max(as.Date(master$END, format = "%m/%d/%Y")) < 
     min(as.Date(MSHQ$START, format = "%m/%d/%Y"))){
  ...
  master_trend <- master %>%
    mutate(`Concatenate for lookup` = paste0(substr(END, 7, 10),
                                             "Q", QUARTER, CPT)) %>%
    left_join(cpt_ref) %>%
    ...
    arrange(DATE) %>%
    pivot_wider(id_cols = c(REP.DEFINITION,CPT.GROUP), 
                names_from = END, values_from = LABOR)
...
upload_master <- function(){
  saveRDS(master, paste0("J:\\deans\\Presidents\\SixSigma\\MSHS Productivity\\",
                         "Productivity\\Volume - Data\\MSH Data\\Charges\\",
                         "Master\\master.rds"))
  write.xlsx(as.data.frame(master_trend), 
             paste0("J:\\deans\\Presidents\\SixSigma\\MSHS Productivity\\",
                    "Productivity\\Volume - Data\\MSH Data\\Charges\\Master\\",
                    "master_trend.xlsx"),
             row.names = F)
  upload <- MSHQ %>% 
    ungroup() %>% 
    select(PARTNER, HOSP, ORACLE.CC, START, END, CPT, QTY, BUDGET)
  upload <<- upload
  start <- format(as.Date(min(MSHQ$START), format = "%m/%d/%Y"), "%Y-%m-%d")
  end <- format(as.Date(max(MSHQ$END), format = "%m/%d/%Y"), "%Y-%m-%d")
  name <- paste0("MSHQ_CPT4_", start, "_", end, ".csv")
  upload_path <- paste0("J:\\deans\\Presidents\\SixSigma\\MSHS Productivity\\",
                        "Productivity\\Volume - Data\\MSH Data\\Charges\\",
                        "Uploads\\", name)
