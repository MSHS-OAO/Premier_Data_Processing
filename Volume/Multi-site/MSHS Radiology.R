...
bislr_or_upload <- rbind(msm_rad_or_data, msw_rad_or_data, msb_rad_or_data, 
  ...
  `colnames<-`(c("Corporation Code", "Entity Code", "Cost Center Code", 
                 ...
                 "Budget Volume"))
## Quality Charts ----------------------------------------------------------
bislr_quality_chart <- rbind(msmw_upload, msbib_upload) %>%
  mutate(`End Date` = as.Date(`End Date`)) %>%
  mutate(qrts = quarters(mdy(bislr_quality_chart$`End Date`)),
        `Concatenate for lookup` = paste0(substr(`End Date`,1,4), qrts, 
                                          `CPT Code & Mod`), 
         Volume = as.numeric(Volume)) %>%
  ...
  left_join(map_cost_centers, by = c("Cost Center" = "Dummy Cost Center")) %>%
  ...
  ungroup() %>%
  group_by(`Cost Center`, `Cost Center Description`, `CPT Group`, 
           `Pay Period End Date`) %>%
  #na.omit() %>% #Removes NA department
  summarise(Vol = sum(True_Volume, na.rm = T)) %>%
  arrange(`Pay Period End Date`) %>%
  pivot_wider(id_cols = c(`Cost Center`, `Cost Center Description`, `CPT Group`),
              names_from = `Pay Period End Date`, values_from = Vol)
# Outputs -----------------------------------------------------------------
