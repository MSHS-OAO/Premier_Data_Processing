...
# User Input --------------------------------------------------------------
pp.start <- as.Date('2022-09-25') # start date of first pay period needed
pp.end <- as.Date('2022-10-22') # end date of the last pay period needed
...
create_rds <- function(x){
  ...
  #table_data <- cbind(File.Table, sapply(file_data, function(z) range(z$Census.Date)))
...
write.table(data_upload_MSMW,
            file = paste0(dir,'/Upload Files', '/MSMW_Census Days_',
                          if(exists('pp.start.new')){format(pp.start.new, '%d%b%y')}else{format(pp.start, '%d%b%y')},
                          ' to ',
                          if(exists('pp.end.new')){format(pp.end.new, '%d%b%y')}else{format(pp.end, '%d%b%y')},
                          ".csv"),
