...
processed_data <- processed_data %>%
  filter(mdy(Earnings.E.D) > min(c(prev_0_max_date_msmw)),
...
write.table(jobcode_list_new, paste0(project_path, "/MSMW/Rightsource Job Code.csv"),
            row.names = F, sep = ",")

# save jobcode dictionary upload
write.table(jc_dict_upload, paste0(project_path, "/MSMW/",
...
write.table(cc_map_fail, paste0(project_path, "/MSMW/",
